...
void incrementa_variavel(relogio *Relogio, relogio *Despertador, calendario *Calendario){
    if(tela_exibida==0){
...
void decrementa_variavel(relogio *Relogio, relogio *Despertador, calendario *Calendario){
    if(tela_exibida==0){
        if(cursor==0) Relogio->hora=Relogio->hora-10;
        if(cursor==1) Relogio->hora--;
        if(cursor==2) Relogio->minuto=Relogio->minuto-10;
...
void checa_botao(relogio *Relogio, relogio *Despertador, calendario *Calendario){
    ...
    if(tela_exibida>1) tela_exibida=0; // TENHO 2 TELAS PARA EXIBIR. 
    
    // INCREMENTAR VARIÁVEL.
    if(PORTDbits.RD1==1){
        __delay_ms(180);  
        incrementa_variavel(&Relogio, &Despertador, &Calendario);        
    ...
    if(PORTDbits.RD2==1){
        __delay_ms(180);   
        decrementa_variavel(&Relogio, &Despertador, &Calendario);
